...
class SenseManager(object):
    def __init__(self, port="/dev/ttyACM0", baudrate=115200):
        ...
        self.foot_contact = np.zeros(4)  # [R_TOE, R_HEEL, L_TOE, L_HEEL]

    def check_contact(self):
        return (self.foot_contact==1).any()
        ...
        while self.ser.in_waiting:
            ...
            if self.ser.read(1)[0] == 0xFF:  # find first byte
                ...
                if self.data4pico[1] == self.data4pico_length_2 and self.data4pico[-2] == self.checksum(self.data4pico[1:-2]) and self.data4pico[-1] == 0xFE:  # check length byte, sum byte & last byte
                    accel=[0,0,0]
                    omega=[0,0,0]
                    accel[0] = self.hex_to_float32(self.data4pico[2:6])
                    accel[1] = self.hex_to_float32(self.data4pico[6:10])
                    accel[2] = self.hex_to_float32(self.data4pico[10:14])
                    omega[0] = self.hex_to_float32(self.data4pico[14:18])
                    omega[1] = self.hex_to_float32(self.data4pico[18:22])
                    omega[2] = self.hex_to_float32(self.data4pico[22:26])
                    for i in range(3):
                        self.accel[i] = self.accel[i]*0.9 + accel[i]*0.1
                        self.omega[i] = self.omega[i]*0.9 + omega[i]*0.1
                    for idx in range(4):
