...
def reportError(msg):
    if msg not in errorStates:
        errorStates[msg]=0

    if errorStates[msg]%300==0:
        print("Error: ", msg)
...
class bear_controller:
    ...
    def start_drivers(self):
        ...
        self.torque_enable_all(0)#sair fix
    ...
    def get_torque_mode(self):
        ...
        try:
            ...
            reportError("Communication Fails!!! 1")
    ...
    def update_present_temperature_and_voltage(self):
        ...
        try:
            ...
            bot.setModuleState(MODULE_COOLING,temp)
            print("BEAR Temperature: %.2f" % temp)
            if temp > 75.0:
            ...
            reportError("Communication Fails!!! 2")
    ...
    def update_present_status(self):
        ...
        try:
            ...
            reportError("Communication Fails!!! 3")
            return False  # communication fails

        # Write to shared memory
        self.bot.setState(LEG_STATE_POS,self.Q)
        self.bot.setState(LEG_STATE_VEL,self.dQF)
        self.bot.setState(LEG_STATE_TOR,self.T)

        return True  # communication succeeds

    def read_write_force(self, positions, velocities, torques):
        """
        ...
        for idx, bear_id in enumerate(BEAR_LIST):
            if bear_goal_values[idx][0] > IQ_MAX:
                printEverySec("max","Setting to max torque for BEAR %02d!!!" % bear_id)
                bear_goal_values[idx][0] = IQ_MAX - 0.5
            elif bear_goal_values[idx][0] < IQ_MIN:
                printEverySec("min","Setting to min torque for BEAR %02d!!!" % bear_id)
        ...
        try:
            ...
            reportError("Communication Fails!!! 4")
            return False  # communication fails
    ...
    def read_write_position(self, positions):
        ...
        try:
            ...
            reportError("Communication Fails!!! 5")
            return False  # communication fails
    ...
    def read_write_torque(self, torques):
        ...
        try:
            ...
            reportError("Communication Fails!!! 6")
            return False  # communication fails

        # Write to shared memory
        data = {"joint_positions": self.Q,
                "joint_velocities": self.dQF}
        BRUCE.Leg_Torq_SMArr[2:12] = self.Q

        return True  # communication succeeds

record={}
def printEverySec(key,text):
...
def main_loop(bot:BRUCE,bc):
    ...
    PosCmd = {"BEAR_enable":  np.array([0.]),
                "BEAR_mode":    np.array([-1.]),
                "goal_torques": np.zeros(10)}
    bot.Leg_Torq_SMArr[0] = 0.0
    bot.Leg_Torq_SMArr[1] = -1
    bot.Leg_Torq_SMArr[2:12] = PosCmd["goal_torques"]
    ...
    while True:
        ...
        PosCmd = bot.All_PosCmd_SMArr
        AllCmd = bot.Value_SMArr
        BEAR_enabled = PosCmd[0]
        BEAR_mode    = PosCmd[1]
        # printEverySec("1",commands)
        # printEverySec("3",bot.LEG_Torq_SMArr)
        if BEAR_enabled == 1.0:
            # if commands["damping"][0] == 1.0:
            #     bc.set_damping_mode()

            # send desired commands to the actuators (MUST SEND COMMANDS BEFORE CHANGING MODE!!!)
            if BEAR_mode == bc.BEAR_modes["force"]:
                #print(AllCmd[TAR_LEG_TOR])
                comm_success = bc.read_write_force(AllCmd[LEG_TAR_POS], AllCmd[LEG_TAR_VEL],AllCmd[LEG_TAR_TOR])
                printEverySec("2","using force mode")
            elif BEAR_mode == bc.BEAR_modes["position"]:
                comm_success = bc.read_write_position(bot.All_PosCmd_SMArr[LEG_TAR_POS])
                printEverySec("2","using position mode")
            elif BEAR_mode == bc.BEAR_modes["torque"]:
                comm_success = bc.read_write_torque(bot.LEG_Torq_SMArr[LEG_TAR_POS])
                printEverySec("2","using torque mode")

            if comm_success is True:
                # check to make sure we are in the correct mode
                if BEAR_mode == bc.BEAR_mode:
                    # enable if not
