...
class dxl_controller:
    ...
    def update_present_status(self):
        ...
        if error == 0:
            ...
            BRUCE.State_SMArr[ARM_STATE_POS]=self.Q_arm
            BRUCE.State_SMArr[ARM_STATE_VEL]=self.dQF_arm
...
def main_loop(dc,bot:BRUCE):
    ...
    bot.All_PosCmd_SMArr[0]=0
    bot.All_PosCmd_SMArr[1]=-1
    ...
    while True:
        ...
        DXL_enable=bot.All_PosCmd_SMArr[0]
        if DXL_enable == 1.0:
            # send desired commands to the actuators
            if dc.DXL_mode == dc.DXL_modes["position"]:
                dc.set_command_position(BRUCE.State_SMArr[ARM_STATE_POS])
        ...
        if present_time > loop_end_time:
            ...
            if delay_time > 1. and frame%1000==0:
