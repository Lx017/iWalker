...
class MujocoViewer:
    ...
    def draw_line(self, line_name, line_data, fig_idx=0, max=100,min=-100):
        fig = self.figs[fig_idx]
        ...
        fig.linepnt[line_idx] = len(line_data)+2
        #fig.linedata[line_idx]=np.random.rand(2000)
        line_data = np.clip(line_data, min, max)
        fig.linedata[line_idx][1:len(line_data)*2:2] = line_data
        fig.linedata[line_idx][len(line_data)*2+1] = max
        fig.linedata[line_idx][len(line_data)*2+3] = min
    ...
    def render(self, camera_body_id=None, readDepth = False,h=0.04,shape=None):
        if self.render_mode == 'offscreen':
            raise NotImplementedError(
        ...
        def update():
            ...
            with self._gui_lock:
                ...
                mujoco.mjv_updateScene(
                    ...
                    self.scn)
                # marker items
                if camera_body_id is not None:
                    i = camera_body_id
                    xmat = self.data.xmat[i].reshape(3, 3).T
                    xpos = self.data.xpos[i]
                    for c in range(2):
                        self.scn.camera[c].pos[:] = xpos
                        self.scn.camera[c].forward[:] = xmat[0]
                        self.scn.camera[c].up[:] = xmat[2]
                        self.scn.camera[c].frustum_top = h
                        self.scn.camera[c].frustum_bottom = -h
                # fwd = self.scn.camera[0].forward
                # pos = self.scn.camera[0].pos
                # up = self.scn.camera[0].up
                #print(fwd, pos, up)
                ...
                if readDepth:
                    ...
                    frustum_bottom = self.scn.camera[0].frustum_bottom
                    frustum_top = self.scn.camera[0].frustum_top
                    frustum_near = self.scn.camera[0].frustum_near
                    frustum_far = self.scn.camera[0].frustum_far
                    return np.flip(depth, axis=0), frustum_near, frustum_far
                # overlay items
                ...
                # vp=mujoco.MjrRect(0,300,30,30)
                # mujoco.mjr_rectangle(vp,1,1,1,0.5)

                glfw.swap_buffers(self.window)
            glfw.poll_events()
            if self._loop_count % 30 == 0:
                realworlddt = time.time() - self.last_render_time
                simdt = self.data.time - self.last_sim_time
                self.timeScale = (simdt / realworlddt)*100 *0.5 + self.timeScale*0.5
                self.last_render_time = time.time()
    ...
    def add_key_hook(self, key, func):
        key = ord(key)
    ...
    def _key_callback(self, window, key, scancode, action, mods):
        ...
        # Slows down simulation
            ...
            self.realMode = "pid" if self.realMode != "pid" else "ctrl"
